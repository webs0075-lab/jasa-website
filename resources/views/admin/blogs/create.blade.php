...
<body class="bg-slate-50">
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            <h2 class="text-3xl font-bold text-slate-900 mb-8">Tambah Blog Baru</h2>
            ...
            <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
                ...
                <form action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-600" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Author</label>
                        <input type="text" name="author" value="{{ old('author') }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-600" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Excerpt (Ringkasan)</label>
                        <textarea name="excerpt" rows="3" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-600" required>{{ old('excerpt') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Konten</label>
                        <textarea name="content" rows="8" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-600" required>{{ old('content') }}</textarea>
                    </div>
