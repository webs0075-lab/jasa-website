...
<body class="bg-slate-50">
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            <h2 class="text-3xl font-bold text-slate-900 mb-8">Edit Portfolio</h2>
            ...
            <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
                ...
                <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Judul Portfolio</label>
                        <input type="text" name="title" value="{{ $portfolio->title }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-600" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Deskripsi</label>
                        <textarea name="description" rows="5" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-600" required>{{ $portfolio->description }}</textarea>
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="category" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-600" required>
                                ...
                                <option value="android" {{ $portfolio->category == 'android' ? 'selected' : '' }}>Android</option>
                                <option value="api" {{ $portfolio->category == 'api' ? 'selected' : '' }}>API</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-900 mb-2">Link (Opsional)</label>
                            <input type="url" name="link" value="{{ $portfolio->link }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-600">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        @if ($portfolio->image)
                            <div class="mb-3">
                                <img src="{{ Storage::url($portfolio->image) }}" alt="{{ $portfolio->title }}" class="w-full max-w-xs rounded-lg">
                            </div>
                        @endif
