    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-3xl font-bold text-slate-900">Portfolio Management</h2>
                <a href="{{ route('admin.portfolios.create') }}" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition font-semibold">
                    + Tambah Portfolio
                </a>
            </div>
                        ...
                        @forelse ($portfolios as $portfolio)
                            <tr class="border-b hover:bg-slate-50">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-sm text-slate-500">{{ Str::limit($portfolio->description, 50) }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('admin.portfolios.edit', $portfolio) }}" class="text-blue-600 hover:underline text-sm mr-4">Edit</a>
                                    <form action="{{ route('admin.portfolios.destroy', $portfolio) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus?')">
                                        @csrf
                                        @method('DELETE')
