...
<body class="bg-slate-50">
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            <h2 class="text-3xl font-bold text-slate-900 mb-8">Edit Testimonial</h2>
                ...
                <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ $testimonial->name }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-emerald-600" required>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-900 mb-2">Posisi</label>
                            <input type="text" name="position" value="{{ $testimonial->position }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-emerald-600" required>
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Perusahaan</label>
                        <input type="text" name="company" value="{{ $testimonial->company }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-emerald-600" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Pesan/Testimoni</label>
                        <textarea name="message" rows="5" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-emerald-600" required>{{ $testimonial->message }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        @if ($testimonial->image)
                            <div class="mb-3">
                                <img src="{{ Storage::url($testimonial->image) }}" alt="{{ $testimonial->name }}" class="w-full max-w-xs rounded-lg">
                            </div>
                        @endif
